...
async function main() {
    const tenantId = process.argv[2] || process.env.TEST_TENANT_ID;

    if (!tenantId) {
        console.error('Error: Debes proporcionar el ID del tenant como argumento o definir TEST_TENANT_ID en .env');
    ...
    try {
        ...
        await prisma.$transaction(
            async (tx) => {
                ...
                // 12. Rituales (Participantes, compromisos y logs se borran en cascada)
